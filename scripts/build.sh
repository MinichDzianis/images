...
WARDEN_IMAGE_REPOSITORY="${WARDEN_IMAGE_REPOSITORY:-"docker.io/stenhigh"}"
...
for file in $(find ${SEARCH_PATH} -type f -name Dockerfile | sort -V); do
    ...
    ## due to build matrix requirements, bitrix specific varients are built in separate invocation
    if [[ ${SEARCH_PATH} == "php-fpm" ]] && [[ ${file} =~ php-fpm/bitrix ]]; then
      continue;
    fi
    if [[ ${SEARCH_PATH} == "php-fpm" ]] && [[ ${file} =~ php-fpm/development ]]; then
      continue;
    fi
